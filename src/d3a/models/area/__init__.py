...
from d3a.models.area.multiprocess_markets import MultiprocessMarketWrapper
...
class Area:
    ...
    def process_event_loop(self, event_queue, market_wrapper, event):
        ...
        while True:
            ...
            if not event_queue.empty() and not event.is_set():
                ...
                self.dispatcher.event_listener(event_type=event_type, **keywordargs)
                self.dispatcher.callback_parent_to_child(event_type=event_type, **keywordargs)
                market_wrapper.write_markets(self.name, self.all_markets)
                event.set()
    ...
    def tick(self):
        if self.current_tick % self.config.ticks_per_slot == 0:
            self._cycle_markets()
        self.dispatcher.broadcast_tick(area_id=self.area_id)
