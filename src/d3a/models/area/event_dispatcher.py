...
class AreaDispatcher:
    ...
    def broadcast_activate(self, **kwargs):
        return self.callback_parent_to_child(AreaEvent.ACTIVATE, **kwargs)

    def broadcast_tick(self, area_id, **kwargs):
        return self.callback_parent_to_child(AreaEvent.TICK, area_id=area_id, **kwargs)

    def broadcast_market_cycle(self, **kwargs):
        return self.callback_parent_to_child(AreaEvent.MARKET_CYCLE, **kwargs)

    def broadcast_balancing_market_cycle(self, **kwargs):
        return self.callback_parent_to_child(AreaEvent.BALANCING_MARKET_CYCLE, **kwargs)

    @property
    def callback_parent_to_child(self):
        if self.area.spawn_process and self.area.is_parent_process:
            return self._broadcast_notification_base_to_sub_process
        else:
            return self._broadcast_notification

    @property
    def callback_child_to_parent(self):
        if self.area.spawn_process is False:
            return self._broadcast_notification
        elif self.area.spawn_process and self.area.is_parent_process:
            return self._broadcast_notification_base_to_sub_process
        else:
            return self._broadcast_notification_sub_to_base_process_blocking
    ...
    def _broadcast_notification_base_to_sub_process(self,
                                                    event_type,
                                                    **kwargs):
        self.area.market_wrapper.write_markets(self.area.name, self.area.all_markets)
        self.area.area_queue.put([event_type, kwargs])

    def _broadcast_notification_sub_to_base_process_blocking(self,
                                                             event_type,
                                                             **kwargs):
        self.area.market_wrapper.write_markets(self.area.name, self.area.all_markets)
        self.area.return_event_queue.put([event_type, kwargs])

    def _broadcast_notification_multiprocessing(self, event_type, **kwargs):
        # Broadcast to children in random order to ensure fairness
        children_to_wait = []
        for child in sorted(self.area.children, key=lambda _: random()):
